...
const defaultDatabaseUrl =
  "postgresql://postgres:postgres@localhost:5432/monorepox";
...
BETTER_AUTH_BASE_URL=${defaultApiUrl}
BETTER_AUTH_SECRET=${authSecret}
ALLOWED_ORIGINS=${defaultWebUrl}
`,
      path: join("apps", "api", ".env"),
    },
    {
      content: `API_URL=${defaultApiUrl}
DATABASE_URL=${defaultDatabaseUrl}
`,
      path: join("apps", "web", ".env.local"),
    },
    {
      content: `DATABASE_URL=${defaultDatabaseUrl}
`,
      path: join("packages", "db", ".env"),
    },
    {
      content: `BETTER_AUTH_SECRET=${authSecret}
`,
      path: join("packages", "auth", ".env"),
    },
  ];
...
const setupDatabase = (packageManager: PackageManagerName) => {
  const filterCommand = packageManager === "npm" ? "--workspace" : "--filter";

  run(packageManager, ["run", "build", filterCommand, "@repo/db"]);
  run(packageManager, ["run", "migrate:deploy", filterCommand, "@repo/db"]);
};

const setupSdk = (packageManager: PackageManagerName) => {
  const filterCommand = packageManager === "npm" ? "--workspace" : "--filter";

  run("bunx", ["turbo", "run", "openapi:export", "--filter=api"]);
  run(packageManager, ["run", "generate", filterCommand, "@repo/sdk"]);
};
...
export const initialize = async (options: {
  ...
  try {
    intro("Let's start a rrr3d project!");
    ...
    if (!options.skipDocker) {
      s.message("Starting local Postgres...");
      try {
        startDatabase();
      } catch {
        log.warn(
          "Could not start Docker. Run `docker compose up -d` before migrating."
        );
      }
    }
    ...
    try {
      ...
      log.warn(
        "Database setup failed. Ensure Postgres is running, then run `bun run migrate:deploy`."
      );
    }

    s.message("Generating SDK...");
    try {
      setupSdk(packageManager);
    } catch {
      log.warn(
        "SDK generation failed. Run `bunx turbo run generate --filter=@repo/sdk` after fixing env."
      );
    }
